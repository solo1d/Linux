...
sudo mkdir ~/Note;
sudo chmod -R 775 ~/Note              #没有给其他人的写权限,只有读和执行权限
sudo chown -R pi:pi ~/Note            #这里我默认用户是 pi,组也是 pi
...
guest ok = yes         #是否运行客人用户进行访问 (也就是不登陆的用户访问)
...
输入密码确定即可。
(注意这个密码是samba独有的,不和系统的用户相同,也没有任何关系)
